...
API_EXPORT int PlayTurn(unsigned int gameTurn, void *ctx, const SGameState *state, STurn *turn)
{
	...
	if (gameTurn == contexte->id) {
		...
		if (vConnexite.size() > 0) {
			...
			//si au moins une cellule à rejoindre
				...
				*/
				if (nbApparition > 1) {
					...
					if (cellDepWithMaxiDices != -1 && cellToATKWithMaxiDices != 1) {
						...
						initDijkstra(map, nbCells, cellDepWithMaxiDices, cellToATKWithMaxiDices, Predecesseurs);//permet de redéfinir le vector de Predecesseurs
						constructWayToFollow(wayToFollow, Predecesseurs, cellDepWithMaxiDices, cellToATKWithMaxiDices);

							
						if ((maxiDice + 1 > state->cells[wayToFollow[wayToFollow.size() - 1]].nbDices) && (maxiDice > 1)) {//si on a au moins 1 dé de plus que l'adversaire
							//on attaque si la cellule attaquée ne nous appartient pas déjà
					...
					if (cellDepartWithMiniDistance != -1 && cellToATKWithMiniDistance != -1) {
						//construction du chemin à suivre
						initDijkstra(map, nbCells, cellDepartWithMiniDistance, cellToATKWithMiniDistance, Predecesseurs);//permet de redéfinir le vector de Predecesseurs
				...
				//si cellToJoin == 0
				//Cas où cellToJoin est vide (1 seule composante connexe)=> dijkstra ne sert a rien et il faut quand meme attaquer
				//on attaque en fonction du nbDes
				if (AttaqueCouteQueCoute(nbCells, contexte->id, state, map, turn)) {
					wayToFollow.clear();
					return 0;
